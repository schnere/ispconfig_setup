...
InstallMTA() {
  case $CFG_MTA in
	...
	"dovecot")
	  ...
	  CFG_OPENDMARC_PWD=$(< /dev/urandom tr -dc A-Z-a-z-0-9 | head -c12)
	  echo "opendmarc opendmarc/dbconfig-install boolean true" | debconf-set-selections
	  echo "opendmarc opendmarc/database-type select mysql" | debconf-set-selections
	  echo "opendmarc opendmarc/mysql/admin-pass password $CFG_MYSQL_ROOT_PWD" | debconf-set-selections
	  echo "opendmarc opendmarc/db/dbname string opendmarc" | debconf-set-selections
	  echo "opendmarc opendmarc/mysql/app-pass password $CFG_OPENDMARC_PWD" | debconf-set-selections
	  echo "opendmarc opendmarc/app-password-confirm password $CFG_OPENDMARC_PWD" | debconf-set-selections
	  echo "opendmarc opendmarc/hosts string localhost" | debconf-set-selections
